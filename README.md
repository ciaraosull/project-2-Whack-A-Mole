...
*   The cartoon moles were edited in Microsof Piant 3D to change to a sad looking mole with a yellow star on its head after a successful hit.  This is to give feedback to the user that their hit was successful.  When the mole reappears it is the original image of the mole until successfully hit again.
...
*   For extra customisation when using a mouse, the cursor was designed to be a cartoon mallet, only around the mole/hole game area.  This adds extra fun and interactivity.  The corordinates of the pointer were configured so they lay to the bottom right where the mallet image looks like it will hit the mole.  The fallback of cursor auto was included in the CSS in case the image fails to load.
